...
CreateDir ConcatPath(BlitzMaxPath(), "/tmp")
...
Function CleanModules( args$[] )
	...
	For name=EachIn mods
		...
		DeleteDir ConcatPath(path, "/.bmx"),True
		
		If Not opt_kill Continue
		
		For Local f$=EachIn LoadDir( path )
		
			Local p$=ConcatPath(path, f)
			Select FileType(p)
			Case FILETYPE_DIR
				If f<>"doc"
					DeleteDir p,True
...
Function SyncModules( args$[] )
	...
	If Sys(ConcatPath(BlitzMaxPath(), "/bin/syncmods"))  Throw "SyncMods error"
