...
Function FmtIncPath$(path$)
?AmigaOS4
	Return " " + CQuote("-I"+AtoU(path))
?Not AmigaOS4
	Return " -I"+CQuote(path)
?
End Function
...
Function MakeMod:TFile( mod_name$ )
	...
	Local src_path$=ConcatPath(path, "/"+id+".bmx")
	Local arc_path$=ConcatPath(path, "/"+id+opt_configmung+".a")
	Local iface_path$= ConcatPath(path, "/"+id+opt_configmung+".i")
	...
	If (mod_name+".").Find(opt_modfilter)=0 And FileType(src_path)=FILETYPE_FILE
		...
		cc_opts:+ FmtIncPath(path)
		cc_opts:+ FmtIncPath(ModulePath(""))
...
Function MakeSrc:TFile( src_path$,buildit )
	...
	If Match( src_ext,"bmx;i" )
		...
		For Local imp$=EachIn src_file.modimports
			...
			cc_opts:+ FmtIncPath(ExtractDir(dep.path))
		...
		For Local imp$=EachIn src_file.imports
			...
			If Match( imp_ext,"h;hpp;hxx;hh" )
				cc_opts:+ FmtIncPath(RealPath(ExtractDir(imp)))
			...
			Else If Match( imp_ext,ALL_SRC_EXTS )
				...
				If EXPERIMENTAL_SPEEDUP And Match( imp_ext,"bmx" )
					Local p$= ConcatPath(ExtractDir( dep.path ), "/.bmx")
					Local i_path$= ConcatPath(p, "/"+StripDir( dep.path )+opt_configmung+".i2")
	...
	If buildit And Match( src_ext,"bmx;c;m;cpp;cxx;cc;mm;s;asm" )
		...
		Local p$=ConcatPath(ExtractDir( src_path ), "/.bmx")
		...
		If FileType( p )<>FILETYPE_DIR Throw "Unable to create temporary directory" +p +"\n"

		Local obj_path$= ConcatPath(p, "/"+StripDir( src_path ))
