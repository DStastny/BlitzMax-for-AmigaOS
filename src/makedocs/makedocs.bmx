...
Function DocBBDocs( docPath$ )
	Local p$=ConcatPath(BmxDocDir, docPath)
	For Local e$=EachIn LoadDir( p )
		Local q$=ConcatPath(p, "/"+e)
		Select FileType( q )
		Case FILETYPE_FILE
			Select ExtractExt( e )
			Case "bbdoc"
			...
			End Select
		Case FILETYPE_DIR
			DocBBDocs ConcatPath(docPath,"/"+e)
		End Select
	Next
...
Function docBmxFile( filePath$,docPath$ )
	...
	Local docDir$=ConcatPath(ExtractDir( filePath ), "/doc")
	...
	For Local line$=EachIn text.Split( "~n" )
		...
		Else If id="import" Or id="include"
			...
			Local p$=ConcatPath(ExtractDir( filePath ),"/"+ParseString( line,i ))
		...
		Else If bbdoc
			...
			If kind
				...
				Local tmpExampleFilePath$ = CasedFileName(ConcatPath(docDir,"/"+id+".bmx"))
