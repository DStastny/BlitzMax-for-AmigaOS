...
ModuleInfo "Amiga edits Doug Stastny"
ModuleInfo "License: zlib/libpng"
ModuleInfo "Copyright: Blitz Research Ltd"
ModuleInfo "Modserver: BRL"

ModuleInfo "History: 1.10 Release"
ModuleInfo "History: AmigaOS RealPath and RootPath handling"
...
?
		Default
			If not _IsRootPath(cd ) And Not cd.EndsWith( "/" ) cd:+"/"
			cd:+ path
			Exit
		End Select
		path=path[i..]
		If Not _IsRootPath( cd )
			Local n= cd.FindLast("/")
?AmigaOS4
			if n = -1 n = cd.FindLast(":")
?
			cd=cd[..n]
		End If
	Wend
