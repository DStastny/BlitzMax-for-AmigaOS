...
BBArray*	bbAppArgs=(BBArray*)BBNULLSTRING;
...
static struct Task*  _mainThread;
static int base_time;

static void startup(){
	_mainThread= IExec->FindTask(0);

	struct timeval tv;
	gettimeofday(&tv,0);
	base_time=tv.tv_sec*1000;
	base_time+=tv.tv_usec/1000;
...
int bbIsMainThread(){
	return IExec->FindTask(0) ==_mainThread;
}
...
void bbStartup( int argc,char *argv[],void *dummy1,void *dummy2 ){
	...
	bbAppDir=&bbEmptyString;
	if (IDOS->NameFromLock(IDOS->GetProgramDir(), buf, PATH_MAX)){
		bbAppDir=bbStringFromCString( buf );
	}
	bbLaunchDir=&bbEmptyString;
    if (IDOS->NameFromLock(IDOS->GetCurrentDir(), buf, PATH_MAX)){
		bbLaunchDir=bbStringFromCString( buf );
	}        
	...
	if (argc == 0) {
		...
		bbAppFile=bbStringFromCString(wb_arg[0].wa_Name);

		bbAppArgs=bbArrayNew1D( "$", argmsg->sm_NumArgs);
		BBINCREFS( bbAppArgs );

		p=(BBString**)BBARRAYDATA( bbAppArgs,1 );
	
		for( k=0;k<argmsg->sm_NumArgs;++k ){

			BBString *arg=bbStringFromCString(wb_arg[k].wa_Name);
			BBINCREFS( arg );
			*p++=arg;
		}
	} else if (argc >= 0) {
		if (IDOS->GetCliProgramName(buf,  PATH_MAX)) {
			bbAppFile=bbStringFromCString(buf);
		}
	...
	if (bbAppFile!=&bbEmptyString){
		bbAppFile=bbStringConcat(bbStringFromCString("/"), bbAppFile);
		bbAppFile=bbStringConcat(bbAppDir, bbAppFile);
	}
    free(buf);
#elif __linux
	int i;
