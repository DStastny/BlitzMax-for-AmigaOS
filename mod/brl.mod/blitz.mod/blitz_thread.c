...
static void *threadProc( void *p ){
	BBThread *thread=p;
        ...
        struct Task* spawner = thread->task;
        thread->task = IExec->FindTask(0);
	...
	BB_UNLOCK
        ...
        IExec->Signal(spawner, SIGF_SINGLE);
	IExec->SetSignal(0, SIGF_SINGLE); 
        IExec->SuspendTask(0, 0);
...
BBThread *bbThreadCreate( BBThreadProc proc,BBObject *data ){
	...
	if( pthread_create( &thread->handle,0,threadProc,thread )>=0 ){
		IExec->Wait(SIGF_SINGLE);
		IExec->SuspendTask(thread->task, 0);
		IExec->Signal(thread->task, SIGF_SINGLE);
		_bbNeedsLock=1;
		return thread;
	}
	free( thread );
...
int bbThreadResume( BBThread *thread ){
	...
	if( n==1 ){
		IExec->RestartTask(thread->task, 0);
	}
...
void _bbThreadUnlockThreads(){
	...
	for( t=threads;t;t=t->succ ){
		if( t!=curThread ){
			if( !--t->suspended ){
				IExec->RestartTask(t->task, 0);
			}
		}
	}
}

// implement a semaphore that takes a count
...
int sem_init(sem_t *sem, int pshared, unsigned int value) {
	if (pshared != 0) {
   	...
   	if (pthread_mutex_init(&sem->mx, NULL) == 0) {
...
void sem_wait(sem_t *sem) {
    ...
    while(sem->count==0) pthread_cond_wait(&sem->cv, &sem->mx);
    sem->count--;
...
void sem_post(sem_t *sem) {
   pthread_mutex_lock(&sem->mx);
   if(sem->count++==0) pthread_cond_broadcast(&sem->cv);
   pthread_mutex_unlock(&sem->mx);
}
