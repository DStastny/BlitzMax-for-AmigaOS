...
void FunBlock::genAssem(){
	...
	string file=concatpath(getdir(opt_infile),".bmx/"+stripdir(opt_infile));
...
void FunBlock::genInterface(){
	...
	if( moduleIdent(opt_module)==stripall(opt_infile) ){
		string file= concatpath(modulePath(opt_module,true), moduleIdent(opt_module)+config_mung+".i");
		ofstream out(file.c_str());
		int k;
		...
		string file=concatpath(getdir(opt_infile), ".bmx/"+stripdir(opt_infile)+config_mung+".i");
		ofstream out(file.c_str());
