...
static void init_env(){
	...
	if (IDOS->NameFromLock(IDOS->GetProgramDir(), path, 1024)==0){
		fail( "NameFromLock" );
	}
	string t=path;
	int n=t.rfind( "/bin" );
	if( n==string::npos ) n=t.rfind( "/BIN" );
	if( n==string::npos ) fail( "Cant find bin:%s",t.c_str() );
	env_blitzpath=t.substr(0,n);
	env_platform="amigaos4";    
}
...
string realpath( string path ){
	...
	fixpath( dir );
	return concatpath(dir, file);
}

string concatpath(string dir, string file)
{
#ifdef __amigaos4__
	string result;
