...
#define r(i) mod_ppc->r[i]
#define f(i) mod_ppc->f[i]
...
CGReg *CGFrame_PPC::genCvt( CGCvt *e ){
	...
	if( r->isint() && e->exp->isint() ){
		...
		const char* fmt = mod_ppc->isApple ? 
			"\tlis\tr0,0x4330\n"
			"\tstw\tr0,__LOCAL+%i(r1)\n"
			"\txoris\t'%i,'%i,0x8000\n"
	...
	if (mod_ppc->isApple) {
		...
		for( k=args.size()-1;k>=0;--k ){
			...
			if( t->isfloat() && fp_id<14 ){
				...
				p=mem(t->type,R[1],arg_sz+mod_ppc->linkagearea );
			}else{
				p=R[arg_sz/4+3];
			}
		...
		int arg_sz=0,gp_id=3, fp_id=1;
		for( k=args.size()-1;k>=0;--k ){
			CGExp *t=args[k],*p;
			if( t->isfloat() && fp_id<9 ){
				p=F[fp_id++];
			}else if(( t->isint()) && gp_id<11  ){
				p=R[gp_id++];
			}else{
				p=mem(t->type,R[1],arg_sz+mod_ppc->linkagearea );	
				arg_sz+=(t->type==CG_FLOAT64) ? 8 : 4;
...
CGReg *CGFrame_PPC::genSym( CGSym *e ){
	CGReg *r=reg(e->type);
	bool isApple = mod_ppc->isApple;
	
	if( e->linkage==CG_IMPORT ){
		string t= isApple ? ""+e->value+"$non_lazy_ptr" : ""+ e->value;
		gen( mov(r,e),
			isApple ?
			"\tlis\t'%i,ha16(%s)\n"
			"\tlwz\t'%i,lo16(%s)('%i)\n"
	...
	"\tla\t'%i,%s@l('%i)\n",
	r->id,e->value.c_str(), r->id,e->value.c_str(),r->id		
...
string CGFrame_PPC::fixSym( string id ){
	if( mod_ppc->isApple ) 	return "_"+id;
	return id;
}
	...
	if (mod_ppc->isApple) {
		...
		for( k=0;k<fun->args.size();++k ){
			...
			if( t->isfloat() && fp_id<14 ){
				...
				CGMem *m=mem(t->type,R[1],arg_sz+mod_ppc->linkagearea  );
				m->flags=MEM_PARAM;
				p=m;
			}else{
				p=R[arg_sz/4+3];
...
*/
	...
	{	
    	...
    	{
		reg_masks[0]=0xfffffff8;	//R0/R1/R2 unavailable!
	//	reg_masks[0]=0x7ffffff8;	//R0/R1/R2/R13 (R31??) unavailable!
		reg_masks[1]=0xfffffffe;	//F0 unavailable!
    	}
	char *buf;
	...
	for( int k=0;k<32;++k ){
		...
		sprintf( buf,rfmt,k<13 ? k : 31+13-k );
